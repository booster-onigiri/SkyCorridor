...
Allow additional disk space for saves and browser cache.
...
The device's home icons are **Observe, Map, Records, YouTube, Friends, Camera**,
read left-to-right from the top row. The short bottom bar returns to its home screen.
...
To use a large video screen, approach its small glowing control tablet and press
**E**. **Esc** returns to the city while playback continues. YouTube needs an internet
connection; availability, ads and selectable quality depend on the provider.
Browser playback is a packaged-game feature.
