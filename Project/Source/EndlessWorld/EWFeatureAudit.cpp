...
void AEWFeatureAudit::Finish(const FString& Failure)
{
    ...
    O->SetStringField(TEXT("scope"),bPad?TEXT("SDL virtual joystick through the production Sony input device, Slate, and CharacterMovement. Physical DualSense USB/Bluetooth and Xbox XInput hardware are not represented."):
        TEXT("Rendered GPU modes; DLSS/Streamline runtime support, active settings and actual frames presented."));
    ...
    UE_LOG(LogTemp,Display,TEXT("EW_FEATURE_DONE %s"),Failure.IsEmpty()?TEXT("PASS"):*Failure);FPlatformMisc::RequestExit(false);
}
void AEWFeatureAudit::Tick(float Delta)
{
    ...
    if(Phase==0)
    {
        ...
        {
            for(int32 SR:{0,1,2,3,4,5})Modes.Add({SR,0,false});
            for(auto M:G->Graphics.FrameGenerationOptions())if(M.Value && M.Supported)Modes.Add({2,M.Value,false});
            if(G->Graphics.CanReconstructRays())Modes.Add({2,0,true});
    ...
    {
        auto E=G->Graphics.Evidence();E->SetNumberField(TEXT("game_frame_ms"),Delta*1000);
        E->SetNumberField(TEXT("mode_index"),Step);E->SetBoolField(TEXT("foreground"),true);
        ...
        Check(E->GetBoolField(TEXT("sr_enabled"))==(Mode.SR!=0),FString::Printf(TEXT("sr_mode_%d_active"),Mode.SR));if(bFinished)return;
        if(Mode.FG && Mode.FG!=10){Check(E->GetNumberField(TEXT("frames_presented"))>=2,FString::Printf(TEXT("fg_%d_presents_generated_frames"),Mode.FG));if(bFinished)return;}
        if(Mode.RR){Check(E->GetBoolField(TEXT("rr_enabled")),TEXT("ray_reconstruction_active"));if(bFinished)return;}
