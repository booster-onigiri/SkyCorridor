...
materials=[]
for family in ['Stone','Edge','Metal','Window','Roof','Glass','Interior','Gold','LitWindow']:
    name='M_Cascade88'+family;path=folder+'/'+name
    ...
    if exists:assert assets.get_metadata_tag(m,'EndlessWorld.Cascade88')=='v1',('unowned material',path)
    ...
    rough={'Stone':.67,'Edge':.51,'Metal':.28,'Window':.115,'LitWindow':.18,'Roof':.24,'Glass':.10,'Interior':.83,'Gold':.32}[family]
    metal={'Metal':.86,'Window':.70,'LitWindow':.4,'Roof':.67,'Gold':.88}.get(family,0)
    ...
    ml.recompile_material(m);assets.set_metadata_tag(m,'EndlessWorld.Cascade88','v1');assert assets.save_loaded_asset(m)
    materials.append(name)

report=[];catalog=json.loads((S/'cascade88-catalog.json').read_text(encoding='utf8'))
for item in catalog['assets']:
    ...
    nanite='Cascade88Glass' not in item['used_materials'];d.build_nanite=nanite
    ...
    for slot in slots:
        ...
        if family not in item['used_materials']:family='Cascade88Stone'
...
unreal.log('EW_CASCADE88_IMPORTED '+str(len(report)))
