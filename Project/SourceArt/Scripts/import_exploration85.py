...
name='M_Explore85Steel'
mat=assets.load_asset(folder+'/'+name) if assets.does_asset_exist(folder+'/'+name) else tools.create_asset(name,folder,unreal.Material,unreal.MaterialFactoryNew())
ml.delete_all_material_expressions(mat);mat.set_editor_property('used_with_instanced_static_meshes',True)
mat.set_editor_property('used_with_nanite',True)
vc=ml.create_material_expression(mat,unreal.MaterialExpressionVertexColor)
assert ml.connect_material_property(vc,'',unreal.MaterialProperty.MP_BASE_COLOR)
for value,prop in [(1,unreal.MaterialProperty.MP_METALLIC),(.32,unreal.MaterialProperty.MP_ROUGHNESS),(.12,unreal.MaterialProperty.MP_ANISOTROPY)]:
    n=ml.create_material_expression(mat,unreal.MaterialExpressionConstant);n.set_editor_property('r',value);assert ml.connect_material_property(n,'',prop)
ml.recompile_material(mat);assets.save_loaded_asset(mat)
report=[]
for item in json.loads((S/'exploration85-catalog.json').read_text())['assets']:
    ...
    nanite=not item['name'].startswith('Explore85Glass');d.build_nanite=nanite
...
(P.parent/'Evidence/exploration85-import-03.json').write_text(json.dumps({'success':True,'assets':report},indent=2),encoding='utf8')
unreal.log('EW_EXPLORATION85_IMPORTED '+str(len(report)))
