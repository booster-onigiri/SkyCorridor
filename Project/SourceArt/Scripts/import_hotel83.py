...
mat=assets.load_asset(folder+'/M_Hotel83Glass') if assets.does_asset_exist(folder+'/M_Hotel83Glass') else tools.create_asset('M_Hotel83Glass',folder,unreal.Material,unreal.MaterialFactoryNew())
...
vc=ml.create_material_expression(mat,unreal.MaterialExpressionVertexColor);ml.connect_material_property(vc,'RGB',unreal.MaterialProperty.MP_BASE_COLOR)
for value,prop in [(.055,unreal.MaterialProperty.MP_OPACITY),(.12,unreal.MaterialProperty.MP_ROUGHNESS),(.48,unreal.MaterialProperty.MP_SPECULAR)]:
    n=ml.create_material_expression(mat,unreal.MaterialExpressionConstant);n.set_editor_property('r',value);ml.connect_material_property(n,'',prop)
ml.recompile_material(mat);assets.save_loaded_asset(mat)
report=[]
for item in json.loads((S/'hotel83-catalog.json').read_text())['assets']:
    ...
    nanite=item['name']!='Hotel83Glass';d.build_nanite=nanite
...
(P.parent/'Evidence/hotel83-import-02.json').write_text(json.dumps({'success':True,'assets':report},indent=2),encoding='utf8')
unreal.log('EW_HOTEL83_IMPORTED '+str(len(report)))
