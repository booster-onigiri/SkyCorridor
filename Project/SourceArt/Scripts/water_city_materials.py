...
CONTRACT="water-city-material-v1"
...
def build_material(family,context):
    ...
    if exists and assets.get_metadata_tag(mat,"EndlessWorld.WaterCity")!=CONTRACT:
        raise RuntimeError("Refusing to replace an unmarked material: "+path)
    rebuild=not exists or assets.get_metadata_tag(mat,"EndlessWorld.WaterCityShaderSHA256")!=source_sha
    if rebuild:
        context["clear_material"](mat)
        mat.set_editor_property("used_with_instanced_static_meshes",True)
        mat.set_editor_property("used_with_nanite",family=="WaterCityStone")
        mat.set_editor_property("tangent_space_normal",False)
        mat.set_editor_property("two_sided",family in ("WaterCityFall","WaterCityFoam"))
        mat.set_editor_property("blend_mode",unreal.BlendMode.BLEND_OPAQUE if family in ("WaterCitySurface","WaterCityStone") else unreal.BlendMode.BLEND_TRANSLUCENT)
        mat.set_editor_property("shading_model",unreal.MaterialShadingModel.MSM_SINGLE_LAYER_WATER if family=="WaterCitySurface" else unreal.MaterialShadingModel.MSM_DEFAULT_LIT)
...
float a=dot(p,float2(1.22,0.64))+T*0.47;
float b=dot(p,float2(-0.53,2.13))-T*0.72;
float c=dot(p,float2(6.70,2.91))+T*1.08;
"""
            inputs={"UV":(uv,""),"Flow":(vc,"RGB"),"T":(time,"")}
            normal=custom("Rebase-stable flowing world normal",common+"""
float2 slope=0.006*cos(a)*float2(1.22,0.64)+0.0025*cos(b)*float2(-0.53,2.13)
            +0.0007*cos(c)*float2(6.70,2.91);
...
""",3,inputs,y=250)
            ...
            output(vector(mat,(.014,.048,.038)),"","MP_BASE_COLOR")
            ...
            water_input("ScatteringCoefficients",vector(mat,(.00024,.00048,.00032)))
            water_input("AbsorptionCoefficients",vector(mat,(.0026,.00125,.0013)))
...
"""
            ...
            colour=custom("Transparent streams with irregular white froth",common+"return lerp(float3(0.23,0.41,0.37),float3(0.84,0.94,0.89),saturate(0.32*stream+0.9*foam));",3,inputs,y=-300)
            alpha=custom("Separated falling ribbons and short foam trails",common+"return edge*(0.028+0.10*stream+0.55*foam);",1,inputs,y=-30)
            normal=custom("Small irregular fall ripples",common+"return normalize(float3((fine-0.5)*0.025,-1.0,(coarse-0.5)*0.016));",3,inputs,y=240)
...
"""
            ...
            output(vector(mat,(.82,.92,.86)),"","MP_BASE_COLOR")
        ...
        assets.set_metadata_tag(mat,"EndlessWorld.WaterCity",CONTRACT)
        assets.set_metadata_tag(mat,"EndlessWorld.WaterCityShaderSHA256",source_sha)
