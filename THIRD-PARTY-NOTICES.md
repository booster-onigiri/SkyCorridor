...
| NVIDIA DLSS / Streamline / Reflex | The NVIDIA Windows build includes runtime components from the unmodified official UE 5.8 DLSS 4.5 plugin 8.7.2. Vendor SDK source and development tools are excluded from the game package and public source. The baseline build uses TSR. | [NVIDIA RTX SDK license](Licenses/NVIDIA-RTX-SDK-LICENSE.txt), [SDK EULA](Licenses/NVIDIA-RTX-SDK-EULA.pdf), and [setup notes](Docs/NVIDIA.md). These components retain NVIDIA's terms. |
